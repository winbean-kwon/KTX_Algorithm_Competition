...
# 상위 200종목 하위 200종목 이렇게 많은 종목을 거래한다면 시장의 흐름이라도 따라가야겠다는 생각

# 1. 변동성을 줄이기 위해 상관관계가 있는 보조지표를 사용하자 -> 테스트 결과 지표들간의 상관관계는 크게 영향을 미치지 못했음
# 2. 그럼 추세를 타는 지표는 어떤게 있을까? MACD, 이평선 OBV, RSI 등등 많이 있지만 하나씩 테스트해본 결과 RSI가 가장 기대치에 부합했음.
# 3. 보조지표를 여러개 사용할 수는 없을까? 2~3개씩 묶어본 결과 RSI와 샤프지수를 합치는게 가장 효율적이었음.
# 4. ARIMA도 넣어볼까? -> 처음엔 그럴듯한 효과를 못봤는데 마지막에 데이터 받고 다시 해보니 괜찮길레 추가
# 5. RSI 지표와 샤프지수는 결과값의 범위도 다르니 -1~1값으로 통일 후 rsi, sharpe, arima 결과값을 다 더했을 때 높은 것으로 분류
# 6. 주어진 데이터의 기간을 통해 나눠서 해보자 -> 장기적으로 상승하거나 하락하는게 큰 변동없이 방향성을 유지할 것 같다
# 7. 기간이 너무 길어서 과거 변동성이 컸던 종목들이 순위에 영향을 미치지 않을까? -> 기간별로 가중치를 둠 (최근 날짜일 수록 높은 가중치)
# 8. 
